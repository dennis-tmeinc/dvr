...
void RC4_crypt(unsigned char * text, int textsize, struct RC4_seed * seed)
{
    ...
    for( n=0; n<textsize; n++) {
        ...
        //       text[n]^=s[ti+tj] ;
        // but it has been used for so long, just leave it as is.
        text[n]^=ti+tj ;
    }
    seed->i=i;
    seed->j=j;
...
inline int _b64(int c)
{
    ...
    else if( c==C64_CODE62 ) {
        return 62 ;
    }
    else if( c==C64_CODE63 ) {
        return 63 ;
    }
    return 0;
}
