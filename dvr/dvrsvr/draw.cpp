...
#define NUM_DRAWAREA (SDK_MAX_MENU_DISP_NUM)

static struct area_t {
    void * id ;
    int x, y, w, h ;
} draw_area [NUM_DRAWAREA] ;

// copy data from draw_buf to MenuBuf of Eagle34
static void draw_refresh_buffer(int x, int y, int w, int h)
{
    ...
    for( iy=y; iy<h; iy++) {
        for( ix=x; ix<w; ix++ ) {
            ...
            if( COLOR_A( pixel ) < 32 ) {
                MenuBufParam.pMenuY[(iy)*MenuBufParam.MenuBufW+ix]=0 ;      // Y=0, U=V=128 make pixel appear transparent, but no color
                if( (iy&1)==0 ) {
                ...
                g1 = COLOR_G( pixel ) ; 
                b1 = COLOR_B( pixel ) ; 
                MenuBufParam.pMenuY[iy*MenuBufParam.MenuBufW+ix]=Y(r1, g1, b1) ;
                if( (iy&1)==0 ) {
                    if( (ix&1)==0 ) {
                        MenuBufParam.pMenuU[(iy/2)*MenuBufParam.MenuBufW+ix]=U(r1, g1, b1) ;
                    }
                    else {
                        MenuBufParam.pMenuU[(iy/2)*MenuBufParam.MenuBufW+ix]=V(r1, g1, b1) ;
                    }
                }
            }
        }
...
void draw_refresh() 
{
    ...
    memset(&MenuRefresh,0,sizeof(MenuRefresh));
    memset(&MenuParam,0,sizeof(MenuParam));

    for( i = 0; i<SDK_MAX_MENU_DISP_NUM ; i++ ) {
        if( draw_area[i].id != NULL ) {

            // copy data to MENUBUF (EAGLE34)
            draw_refresh_buffer(draw_area[i].x, draw_area[i].y, draw_area[i].w, draw_area[i].h) ;
            
            // also refresh video out
            MenuRefresh.x =draw_area[i].x;
            MenuRefresh.y =draw_area[i].y;
            MenuRefresh.w =draw_area[i].w;
            MenuRefresh.h =draw_area[i].h;
            RefreshMenuDisp(&MenuRefresh);

            // add refresh area
            MenuParam.DispParam[i].bEnable = 1;
            MenuParam.DispParam[i].menuAlpha = SDK_ALPHA_MENU_BACK31;
            MenuParam.DispParam[i].x = draw_area[i].x;
            MenuParam.DispParam[i].y = draw_area[i].y;
            MenuParam.DispParam[i].w = draw_area[i].w;
            MenuParam.DispParam[i].h = draw_area[i].h;
...
void draw_show( void * id, int x, int y, int w, int h )
{
    ...
    for( i=0; i<NUM_DRAWAREA; i++ ) {
        if( draw_area[i].id == id ) {
            draw_area[i].id=NULL ;
        }
    }
    for( i=0; i<NUM_DRAWAREA; i++ ) {
    ...
    if( i>=NUM_DRAWAREA ) {
        for( i=NUM_DRAWAREA-1 ; i>0 ; i-- ) {
            draw_area[i] = draw_area[i-1] ;
        }
        i=0;
    }
...
void draw_fillline( int x, int y, int w )
{
	// check drawing limit
