...
static struct timeval timetick_ref;

void time_init()
{
    time_inittimezone();
	if( g_timetick==0 ) {
		gettimeofday( &timetick_ref, NULL );
	}
}
...
int time_tick()
{
	...
	if( tv.tv_sec> timetick_ref.tv_sec ||
	   ( tv.tv_sec==timetick_ref.tv_sec && tv.tv_usec>timetick_ref.tv_usec ) ) 
	{
		g_timetick+=
			(tv.tv_sec-timetick_ref.tv_sec)*1000 + 
			(tv.tv_usec-timetick_ref.tv_usec)/1000 ;
	}
	timetick_ref = tv ;
