...
void tvsid_page()
{
    ...
    tvsid_filename = getenv( "POST_FILE_tvsid_file" );
    if( tvsid_filename!=NULL ) {
        tvsid_file = fopen( tvsid_filename, "r" ) ;
        if( tvsid_file ) {
            // now we check file format
            char * usbid = new char [400] ;
            char * c_tvskey = new char [4096] ;
            struct key_data * tvskey = (struct key_data *)c_tvskey ;
            int keysize=0 ;
            fread( usbid, 1, 256, tvsid_file ) ;
            usbid[256]=0;
            keysize = fread( c_tvskey, 1, 4096, tvsid_file ) ;
            if( checktvskey( usbid, tvskey, keysize ) ) {
                if( tvskey->usbid[0] == 'M' && tvskey->usbid[1] == 'F' ) {
                    config dvrconfig(dvrconfigfile);    
                    dvrconfig.setvalue( "system", "tvsmfid", tvskey->usbid );
                    bin2c64((unsigned char *)(tvskey->videokey), 256, usbid);		// convert to c64
...
// write down reports
    tvsid_file = fopen( "tvsid.upd", "w" ) ;
    if( tvsid_file ) {
        if( res ) {
            // success
            fprintf( tvsid_file, "TVS Manufacturer ID file upload success!" );
        }
        else {
            // failed
            fprintf( tvsid_file, "TVS Manufacturer ID file upload failed!" );
        }
        fclose( tvsid_file );
    }
    if( res ) {
        tvsid_file = fopen( "/var/dvr/dvrsvr.pid", "r" ) ;
        if( tvsid_file ) {
            dvrpid=0;
            fscanf(tvsid_file, "%d", &dvrpid ) ;
            if( dvrpid>0 ) {
...
int main()
{
    ...
    else if( page && strcmp(page, "/tvsid.html")==0 ) {
        tvsid_page();
    }
