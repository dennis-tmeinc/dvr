...
struct http_cfg {
	...
	char * tmpdir ;					// temporay dir for post file
	char * alt_root ;
	char * default_page ;
	char * denied_page ;
} ;
...
static void cfg_init()
{
	...
	cfg.tmpdir = NULL ;
	cfg.alt_root = strclone( "/davinci/dvr/www" );
...
// return value
//		0: not exist, 1: in document root, 2: in alt document root, 3: in gz
...
// flush completly, include file buffer
void http_flush()
{
	// do buffer flush out first
	http_buffer_flush();
}
...
{
    struct stat filest ;
    if( http_cachemaxage > 0 && file != NULL && document_stat( file, &filest ) ) {
...
int http_inputmultipart( int content_length )
{
    ...
    while(ulbytes<content_length) {
        ...
        if( cfg.tmpdir != NULL ) {
			sprintf(part_name, "%s/post_file_%d_%d",  cfg.tmpdir, (int)getpid(), partnum );
		}
		else {
...
void http_document()
{
			  ...
			  strcasecmp(&uri[len-4], ".cgi")==0 ))
    {
		...
		if( doctype==1 ) {			// no cache for files on doc root 
			http_nocache();
		}
		else {
		...
		while( fgets( lbuf, HTTP_LINESIZE, cfgfile ) ) {
			...
			if( xvalue ) {
				*xvalue = 0 ;
				...
				else if(strcmp( xkey, "httpd.tmpdir" ) == 0 ) {
					if( cfg.tmpdir ) free( cfg.tmpdir ) ;
					cfg.tmpdir = strclone( trim(xvalue) ) ;
				}		
	...
	for( i=1; i<argc; i++ ) {
		...
		if( *ar == '-' ) {
			switch( ar[1] ) {
				...
				case 't' :			// temp post dir
					if( cfg.tmpdir ) free( cfg.tmpdir ) ;
					cfg.tmpdir = strclone( ar+2 ) ;
					break;
