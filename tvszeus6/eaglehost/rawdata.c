...
int send_raw_packet(struct my_message *payload,
		...
		uint32_t dest_ip, int dest_port, int ifindex)
{
...
int main()
{
    ...
    struct my_message mymsg, recvmsg ;	
	...
	sock.sll_ifindex = 0 ;
	res = bind(fd, (struct sockaddr *) &sock, sizeof(sock));
    close( fd );
	...
	addr.sin_port = htons(15114);
	bind(fd, (struct sockaddr *)&addr, sizeof(addr));
    memset( &recvmsg, 0, sizeof(recvmsg) );
    strcpy( mymsg.msg, "hello!xxx" );
    send_raw_packet(&mymsg, INADDR_ANY, 15115, INADDR_BROADCAST, 15114, 1 );
