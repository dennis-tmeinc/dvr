...
static int s_pwii_output = 0 ;
static int s_pwii_emg_time = 0  ;
...
{	
	...
	// auto turn off EMG input
	if( (p_dio_mmap->inputmap | PWII_MIC1_EMG )!=0 && (runtime - s_pwii_emg_time ) > 2000 ) {
		p_dio_mmap->inputmap &= ~ PWII_MIC1_EMG  ;
	}

	if( (p_dio_mmap->inputmap | PWII_MIC2_EMG )!=0 && (runtime - s_pwii_emg_time ) > 2000 ) {
		p_dio_mmap->inputmap &= ~ PWII_MIC2_EMG  ;
	}
	
	int mic ;
	// mic1
	mic = p_dio_mmap->pwii_output & PWII_MIC1_ON  ;
	if( mic != ( s_pwii_output & PWII_MIC1_ON) ) {
		if( mic ) {
			mcu_mic_on( 0 );			
		}
		else {
	...
	mic = p_dio_mmap->pwii_output & PWII_MIC2_ON  ;
	if( mic != ( s_pwii_output & PWII_MIC2_ON) ) {
		if( mic ) {
			mcu_mic_on( 1 );			
		}
		else {
	...
	// covert mode ?
	int  covert = p_dio_mmap->pwii_output & PWII_COVERT_MODE ;
	if( covert != ( s_pwii_output &  PWII_COVERT_MODE )) {
		if( covert == 0 ) {
			mcu_covert( 0 ) ;
	...
	s_pwii_output = p_dio_mmap->pwii_output  ;
