	  ...
	  diff=time0.tv_sec-lastturnontime;
		...
		if(time1.tv_sec-time0.tv_sec>10)
	      ...
	      if(isHarddiskup()){
			...
			gettimeofday(&time1, NULL);
			if(time1.tv_sec-time0.tv_sec>180)
			  break;
			dio_kickwatchdog();
		    }		
	      }
...
int disk_findrecdisk(int prerun)
{
    ...
    if(gHBD_Recording){
 	...
 	for( loop=0; loop<50; loop++ ) {
	    ...
	    if(disk_size(disk_disklist[recordingDisk].basedir.getstring())<10*1024)
	    {
...
int isHarddiskup()
{
    ...
    if(number==2){
      dvr_log("hard disk is powered on");     
      return 1;
    } 
    return 0;
}
 ...
 /* if so, turn on the hard disk */
  ...
  for(i=0;i<10;++i){
	...
	if(harddiskup){
		...
		gettimeofday(&time1, NULL);
		if(time1.tv_sec-time0.tv_sec>360) 
		  break;
		dio_kickwatchdog();
	    }	
	    break;
