...
void capture::loadconfig()
{
    ...
    m_attr.SecondStreamEn=dvrconfig.getvalueint( section, "second_enable");
    m_attr.FrameRate_Second=2;//dvrconfig.getvalueint( section, "framerate_second");  
    m_attr.Resolution_Second=1;//dvrconfig.getvalueint( section, "resolution_second");
    m_attr.Liveview_stream=1;//dvrconfig.getvalueint( section, "liveview_stream");
    m_attr.SecondRecordingEn=dvrconfig.getvalueint( section, "secondrecording_enable");
    ...
    m_attr.Bitrate_Second=1000000;//dvrconfig.getvalueint( section, "bitrate_second");
    ...
    m_attr.disableaudio = 0;//dvrconfig.getvalueint( section, "disableaudio" );    
...
void capture::onframe(cap_frame * pcapframe)
{
    ...
    if(pcapframe->frametype==FRAMETYPE_VIDEO_SUB||pcapframe->frametype == FRAMETYPE_KEYVIDEO_SUB){
	    ...
	    pcapframe->frametype = FRAMETYPE_KEYVIDEO;
         ...
         if((m_attr.Liveview_stream==LIVEVIEW_SUB)&&m_attr.SecondStreamEn){
	    net_onframe(pcapframe);
	 }
	 /*
	 if(m_attr.SecondRecordingEn){
	 ...
	 */
         ...
         if((m_attr.Liveview_stream==LIVEVIEW_MAIN)||!m_attr.SecondStreamEn){
	    net_onframe(pcapframe);
	 }         
         rec_onframe(pcapframe); 
	 ...
	 net_onframe(pcapframe);
         rec_onframe(pcapframe);      
	 /*
	 if(m_attr.SecondStreamEn&&m_attr.SecondRecordingEn){
	    pcapframe->channel+=8; 
	    rec_onframe(pcapframe);  	     
	 }*/
    }
