...
void eagle_capture::streamcallback(CALLBACK_DATA CallBackData)
{
    ...
    case FRAME_TYPE_SUBVIDEO_I:	
        xframetype=FRAMETYPE_KEYVIDEO_SUB;
      break;
    case FRAME_TYPE_SUBVIDEO_P:
        xframetype=FRAMETYPE_VIDEO_SUB;
      break;
    default:
        return;
    }
...
printf("Audio: phy %d, s %d\n", 	CallBackData.channel,  CallBackData.size );		
	...
	if(xframetype==FRAMETYPE_KEYVIDEO||xframetype==FRAMETYPE_KEYVIDEO_SUB){		  
	  //copy the osd to
	    int i;
	    int bufNum=0; 
	    int size;
	...
	} else if(xframetype==FRAMETYPE_VIDEO_SUB){  
	    if(mOSDChanged_Sub){
		int i;
		int bufNum=0; 
		int size;
...
printf("Ch: %d, Q: %d\n", m_hikhandle-1, QuantVal );
	...
	//dvr_log("set Bitrate Control");
       ...
       const static picture_format fmt[3] = {
        ...
        else if( m_attr.Resolution>5 ) {
	  if (m_attr.Resolution != 0xf)
            m_attr.Resolution = 3 ;
        }
	//m_attr.Resolution = 5 ; // harry: testing
    //    dvr_log("set picture resolution");
        res = SetEncoderPictureFormat(m_hikhandle-1, m_chantype, m_attr.Resolution);
 
	SetSecondStreamEnable(m_hikhandle-1,m_attr.SecondStreamEn);	
	if(m_attr.SecondStreamEn){
            SetSecondStreamBitrate(m_hikhandle-1,m_attr.Bitrate_Second);
	    SetSecondStreamFramerate(m_hikhandle-1,m_attr.FrameRate_Second);
	    SetSecondStreamResolution(m_hikhandle-1,m_attr.Resolution_Second);	
	    SetSecondStreamInterval(m_hikhandle-1,m_attr.FrameRate_Second*3);
	}
#if 0	
