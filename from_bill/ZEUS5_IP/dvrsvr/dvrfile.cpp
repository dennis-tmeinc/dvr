...
#define WRITE_BUFFER_SIZE (1024*512)
...
dvrfile::dvrfile()
{
  ...
  //  m_filebuf = NULL;
    ...
    m_syncsize=0 ;
    m_framepos=0 ;
    pWriteBuff=(char*)malloc(WRITE_BUFFER_SIZE);
    WriteBuffSize=0;
}
...
void dvrfile::close()
{
    ...
    if (isopen()) {
		...
		if( m_keyarray.size()>0 ) {
            ...
            file_syncreq = 1;
	    if(WriteBuffSize>0){
	       write(pWriteBuff,WriteBuffSize);
	    }
	    WriteBuffSize=0;
         //   sync();
	    fdatasync( fileno(m_handle) );
...
int dvrfile::writeframe(void *buffer, size_t size, int keyframe, dvrtime * frametime)
{
    ...
    if (size > 0) {
       if(WriteBuffSize+size<WRITE_BUFFER_SIZE){
	 memcpy(pWriteBuff+WriteBuffSize,wbuf,size);
	 WriteBuffSize+=size;
	 return 1;
       } else {
	 int left=WRITE_BUFFER_SIZE-WriteBuffSize;
	 memcpy(pWriteBuff+WriteBuffSize,wbuf,left);
	 if(write(pWriteBuff,WRITE_BUFFER_SIZE)==WRITE_BUFFER_SIZE){
	    memcpy(pWriteBuff,wbuf+left,size-left); 
	    WriteBuffSize=size-left;
	    return 1;
	 } else {
	    memcpy(pWriteBuff,wbuf+left,size-left); 
	    WriteBuffSize=size-left;
