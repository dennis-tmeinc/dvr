...
int setRequestFileName()
{
   char buf[256];
   FILE *fp = fopen("/var/dvr/dvrcurdisk", "r");
   if(fp){
      fscanf(fp,"%s",buf);      
      sprintf(g_requestfilename,"%s/markrequest",buf);
      fclose(fp);
   } 
   return -1;
}
...
void checkpreviousrequestmark()
{
    time_t from,to;
    FILE* fr=fopen(g_requestfilename,"r");
    if(fr){
       while(fscanf(fr,"%d,%d\n", &from, &to)==2){
	 add_mark_entry_to_list(&g_rml, from, to);
       }
       fclose(fr); 
    }
}

void removerequestmark()
{
    unlink(g_requestfilename); 
}

void addmarkentrytomarkfile(time_t from,time_t to)
{
   FILE* fw=fopen(g_requestfilename,"a");
   if(fw){
      fprintf(fw,"%d,%d\n", from,to);
      fclose(fw);
   }
  ...
  while ((end = strchr(start, '\n'))) {
    *end = '\0';
	...
	if (6 == sscanf(strtime2,
	  ...
	  addmarkentrytomarkfile(mktime(&from),mktime(&to));
	}
      }
    }
