...
int main()
{
	...
	value = dvrconfig.getvalue("system", "numberofdiskused");
	if( value.length()>0 ) {
	    fprintf(fvalue, "\"disk_number\":\"%s\",", value.getstring() );
	}             

	value = dvrconfig.getvalue("system", "partitionperdisk");
	if( value.length()>0 ) {
	    fprintf(fvalue, "\"partitions_perdisk\":\"%s\",", value.getstring() );
	}      
    ...
    system("cp /mnt/nand/dvr/firmwareid ./firmware_version");
