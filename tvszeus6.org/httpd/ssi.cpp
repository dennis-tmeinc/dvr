...
void http_response( int status, char * title, char * mime_type, int length)
{
    time_t now;
    char hbuf[120];
    int i ;
    ...
    if( fn ) {
        ...
        if( fexhdr ) {
            while( fgets( hbuf, sizeof(hbuf), fexhdr ) )
            {
                if( (fn=strchr(hbuf, ':' )) ) {
                    *fn=0 ;
                    http_setheader(stringtrim(hbuf), stringtrim(fn+1)) ;
                }
            }
            fclose( fexhdr );
        }
...
void http_error( int status, char * title )
{
    ...
    http_response( status, title, "text/html", -1 );
...
int http_checkcache()
{
    ...
    //  etag
    sprintf(etagstr, "\"%x\"", (unsigned long)http_etag);
    ...
    if( childpid==0 ) {
        ...
        while ( fgets( cgibuf, sizeof(cgibuf), fp )  )
        {
            if( cgibuf[0] < ' ' ) {
                ...
                *p=0 ;
                http_setheader(stringtrim(cgibuf), stringtrim(p+1)) ;
            }
        }

        http_response( 200, NULL, NULL, len-ftell(fp) );
...
void smallssi_run()
{
    ...
    if( http_checkcache() ) {       // check if cache fresh?
        // use cahce
        http_response( 304, NULL, NULL, -1 );        // let browser to use cache
    }
    else {
        fp = fopen( ssioutfile, "r" );
        fseek( fp, 0, SEEK_END );
        http_response( 200, NULL, "text/html; charset=UTF-8", ftell(fp) );
        fseek( fp, 0, SEEK_SET );
        int ch ;
        while( (ch=fgetc(fp))!=EOF ) {
            fputc( ch, stdout );
...
int savepost()
{
    ...
    if( strcmp( request_method, "POST" )==0 &&      // support "POST" contents only
        ...
        content_length<50000000 )                    // can't handle huge contents
    {
...
void http_document()
{
             ...
             strcasecmp(&fname[len-4], ".cgi")==0 )
    {
        ...
        cache_upd( fname );
        if( http_checkcache() ) {       // check if cache fresh?
            http_response( 304, NULL, NULL, -1 );        // to let browser use cache
            return ;
        }
        ...
        http_response( 200, NULL, get_mime_type( fname ), len );
...
void http()
{
	...
	socklen_t slen ;
	slen = sizeof(sname);
	if( getsockname(0, (struct sockaddr *)&sname, &slen)==0 ) {
SERVER_ADDR="192.168.1.198"
SERVER_NAME="192.168.1.198"
SERVER_PORT="80"
	}
	slen = sizeof(sname);
	if( getpeername(0, (struct sockaddr *)&sname, &slen)==0 ) {
...
int main(int argc, char * argv[])
{
	...
	setenv( "SERVER_PROTOCOL", PROTOCOL );
    chdir(document_root);
