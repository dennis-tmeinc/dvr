...
cfg_init_sections(void)
{
    ...
    c->sec_names[0]   = NULL;
    c->sec_entries    = (struct CFG_ENTRIES **)
      malloc(ALLOC_SIZE*sizeof(struct CFG_ENTRIES*));
...
cfg_find_section(struct CFG_SECTIONS *c, char *name)
{
	...
	realloc(c->sec_names, (c->sec_count+2+ALLOC_SIZE)*sizeof(char*));
      c->sec_entries = (struct CFG_ENTRIES **)
	realloc(c->sec_entries,
		(c->sec_count+2+ALLOC_SIZE)*sizeof(struct CFG_ENTRIES*));
    }
    e = cfg_init_entries();
...
cfg_parse_file(char *filename)
{
    ...
    char line[512],tag[64],value[448];
    FILE *fp;
    int nr;
