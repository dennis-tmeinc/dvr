...
static void OpenSerialPort(ISP_ENVIRONMENT *IspEnvironment)
{
    ...
    DebugPrintf(3, "COM-Port %s opened...\n", IspEnvironment->serial_port);
    DebugPrintf(2, "COM-Port %s opened at rate:%d...\n", IspEnvironment->serial_port,atol(IspEnvironment->baud_rate));
    ...
    if (argc >= 5)
    {
        for (i = 1; i < argc - 3; i++)
        {
            ...
            if(strnicmp(argv[i],"-try", 4) == 0)
            {
                ...
                num = argv[i] + 4;
                if(isdigit(*num) != 0) {
					IspEnvironment->nQuestionMarks = atoi( num);
                	DebugPrintf(3, "Try %d times to synchronise\n", IspEnvironment->nQuestionMarks );
				}
                continue;
            }
