...
static void disk_removefile( const char * file264 ) 
{
    ...
    f264 = file264 ;
    remove( file264 );
    extension=strstr( f264.getstring(), ".265" );
    if( extension ) {
        strcpy( extension, ".k");
...
int disk_olddestfile( char * dir,  struct dvrtime *oldesttime, string * oldestfilename, int lock )
{
    ...
    while( dfind.find() ) {
        ...
        if( dfind.isfile() ) {
            filename = dfind.filename();
            l = strlen(filename) ;
            if( l>4 && strcmp(&filename[l-4], ".265")==0 ) {    // is it a .264 file ?
...
void disk_scandayinfo( char * daydir, disk_day_info * ddi )
{
    ...
    while( dfind.find() ) 
    {
        ...
        if( dfind.isfile() ) {
            filename = dfind.filename();
            l = strlen(filename) ;
            if( l>4 && strcmp(&filename[l-4], ".265")==0 ) {    // it is a .264 file
                f264time(filename, &dvrt);
        ...
        if( disks.isdir() ) {
            ...
            if( stat(diskname, &diskstat)==0 ) {
                if( diskstat.st_dev != basestat.st_dev) {	// found a mount point
                    ...
                    for( i=0; i<disk_disklist.size(); i++ ) {
			// already in list
                        if( disk_disklist[i].dev == diskstat.st_dev ) {    
                            disk_check=1;
			       ...
			       struct fileInfo *fi, char *servername)
{
  ...
  snprintf(path, size,
	   ...
	   (fi->ext == 'k') ? "k" : 
	   (dvr_type == DVR_TYPE_DM510) ? "265" : "mp4");
