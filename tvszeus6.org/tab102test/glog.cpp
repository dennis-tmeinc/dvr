...
#define gps_clear() serial_clear(gps_port_handle)
#if 1
...
int gps_readdata(struct gps_data * gpsdata)
{
  char line[300] ;		
  int l;
  if( (l=gps_readline( line, sizeof(line) ))>0 ) {
    static int gpsfifo=0 ;
    if( gpsfifo<=0 ) {
      gpsfifo=open( "/davinci/gpsfifo", O_WRONLY | O_NONBLOCK );
    }
    if( gpsfifo>0 ) {
      int t;
      char str[300];
      t = sprintf(str, "%d bytes", l);
      if( write( gpsfifo, str, t )!=t ) {
	close(gpsfifo);
	gpsfifo=0;
      }
    }
...
int appinit()
{
	...
	strcpy( gps_port_dev, "/dev/ttyUSB1" );
	i = 4800;//dvrconfig.getvalueint("glog", "serialbaudrate");
	if( i>=1200 && i<=115200 ) {
		gps_port_baudrate=i ;
	}
...
#ifdef FILE_DEBUG
		    if (!gps_valid) {
		      writeDebug("gps valid");
#ifdef NET_DEBUG
		      writeNetDebug("gps valid");
#endif
		    }
		    gps_valid = 1;
#endif
...
#ifdef FILE_DEBUG
		if (gps_valid) {
		  writeDebug("gps not valid");
#ifdef NET_DEBUG
		  writeNetDebug("gps not valid");
#endif
		}
		gps_valid = 0;
#endif
