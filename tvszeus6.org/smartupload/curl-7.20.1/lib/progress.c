...
int Curl_pgrsUpdate(struct connectdata *conn)
{
  ...
  data->progress.dlspeed = (curl_off_t)
    ((double)(long)data->progress.downloaded/
     (data->progress.timespent>0?data->progress.timespent:1)); //harry

  /* The average upload speed this far */
  data->progress.ulspeed = (curl_off_t)
    ((double)(long)data->progress.uploaded/
     (data->progress.timespent>0?data->progress.timespent:1)); //harry
      ...
      /* Calculate the average speed the last 'span_ms' milliseconds */
      {
        ...
        if(amount > CURL_OFF_T_C(4294967) /* 0xffffffff/1000 */)
          ...
          data->progress.current_speed = (curl_off_t)
            ((double)(long)amount/((double)span_ms/1000.0)); //harry
        else
  ...
  if(!(data->progress.flags & PGRS_HIDE)) {
    ...
    if(data->set.fprogress) {
      ...
      result= data->set.fprogress(data->set.progress_client,
                                  (double)(long)data->progress.size_dl,
                                  (double)(long)data->progress.downloaded,
                                  (double)(long)data->progress.size_ul,
                                  (double)(long)data->progress.uploaded); //harry
      if(result)
