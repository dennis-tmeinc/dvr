    ...
    if( page && strcmp(page,"status")==0) {     // originate from status page ?
        ...
        if( synctime ) {
            ...
            if( stime>0 ) {                     // milliseconds since 1970/01/01
                ...
                system( "/dav/dvr/dvrtime utctomcu > /dev/null" );
                system( "/dav/dvr/dvrtime utctortc > /dev/null" );
            }
        }
#if 0        
...
void firmwareupload_page()
{
    ...
    if( firmwareok ) {
        ...
        if( fork()==0 ) {
            ...
            system( "mkdir upfirmware; cd upfirmware; ../firmware_x/fw ;sync ; sleep 2 ;reboot" );
            exit(0);
        }
    }
}
...
void tab102firmwareupload_page()
{
    ...
    if( tab102_upd_pid == 0 ) {
        ...
        execlp("/dav/dvr/tab102", "tab102", "-fw", tab102_firmwarefilename, NULL );
        exit(0);
    }
...
void mcu_firmwareupload_page()
{
    ...
    if( mcu_upd_pid == 0 ) {
        ...
        // updating MCU firmware
        execlp("/dav/dvr/ioprocess", "ioprocess", "-fw", mcu_firmwarefilename, NULL );
        exit(0);
    }
