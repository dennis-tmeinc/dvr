 ...
 * 07/08/2011 by Harrison
 *   1. leave network power on if wifi is disabled.

 * 09/07/2011 by Harrison
 *   1. added vter support.

 * 10/26/2011 by Harrison
 *   1. separate timeout for HD detection & mount failure.
 */
...
#define KEY_BOUNCE_BASE 2
#define SENSOR_DEBOUNCE 1000 /* millisec */
...
struct sensor_t g_sensorConf[MAXINPUT];

unsigned int g_input_debounced, g_input_save; /* only mcu inputs */
...
int g_mcuinputnum, g_mcuinputmask;
...
unsigned int clear_masked_inputbits(unsigned int bitmap, unsigned int mask)
{
  return bitmap & ~mask;
}

static void copyInputBit(unsigned int *dst, unsigned int *src, int bit)
{
  unsigned int mask;
...
void update_inputmap()
{
  ...
  unsigned int imap;
  int i;
  imap = p_dio_mmap->inputmap ;
  for( i=0; i<g_mcuinputnum; i++ ) {
    copyInputBit(&imap, &g_input_debounced, i);
  }
...
unsigned int translate_mcu_inputs(unsigned int mcu_bitmap)
{
  int i;
  unsigned int imap = 0;
...
/* check masked sensor inputs (already remapped)
 * return:  bitmap of changed inputs(including event marker).
 *          changed input bit: 1
 */
static unsigned int checkSensorInputs(unsigned int input, unsigned inputmask)
{
  int i;
  unsigned int mask;
  struct timeval tv;
  unsigned int bitmap = 0, val;

  gettimeofday(&tv, NULL);

  val = g_input_debounced;

  for (i = 0; i < g_mcuinputnum; i++) {
    mask = 1 << i;
    if ((mask & inputmask) == 0)
      continue;
    /* compare with inputSave to update the lastChanged time */
    if ((g_input_save & mask) != (input & mask)) {
      /* check last change time */
      if (g_sensorConf[i].lastChanged.tv_sec == 0) {
	//fprintf(stderr, "  bit %d changed\n", i);
	bitmap |= mask;
	copyInputBit(&val, &input, i);
      } else {
	if (timeval_diff(&tv, &g_sensorConf[i].lastChanged) >=
	    SENSOR_DEBOUNCE) {
	  //fprintf(stderr, "  bit %d changed2\n", i);
	  bitmap |= mask;
	  copyInputBit(&val, &input, i);
	  g_sensorConf[i].lastChanged.tv_sec = 0;
	  continue; /* don't set the lastChanged time */
	}
      }
      g_sensorConf[i].lastChanged = tv;
    }
  }

  g_input_debounced = val;

  return bitmap;
}

/* check sensor debouncing time out.
 */
static unsigned char checkSensorInputDebouncing()
{
  int i;
  unsigned char mask;
  ...
  for (i = 0; i <  g_mcuinputnum; i++) {
    mask = 1 << i;
    /* check last change time */
    if (g_sensorConf[i].lastChanged.tv_sec &&
	(timeval_diff(&tv, &g_sensorConf[i].lastChanged) >=
	 SENSOR_DEBOUNCE)) {
      g_sensorConf[i].lastChanged.tv_sec = 0;
      if ((g_input_save & mask) != (g_input_debounced & mask)) {
	bitmap |= mask;
	copyInputBit(&g_input_debounced, &g_input_save, i);
      }
    }
  }
  ...
  */
  ...
  while (1) {
	...
	continue;
      ...
      unsigned int imap1 = translate_mcu_inputs((buf[6] << 8) | buf[5]);
      unsigned int changed_bitmap = checkSensorInputs(imap1, g_mcuinputmask);
      dprintf("DI change:%X,%02X,%02X\n", changed_bitmap,buf[5],buf[6]);
      if (changed_bitmap)
	update_inputmap();
      g_input_save = imap1 & g_mcuinputmask;
    } else if ((buf[0] == 0x07) &&
	       (buf[1] == 0x00) &&
	       (buf[2] == 0x01) &&
...
int mcu_bootupready()
{
  ...
  while (retry > 0) {
	...
	if (verifyChecksum(buf, buf[0])) {
	  ...
	  hdlock = ((buf[5] & HDKEYLOCK) == 0);	  // HD locked
	  hdinserted = ((buf[5] & HDINSERTED) == 0);  // HD inserted
	  /* MCU gives DI for lower 6 bits only on boot ready. */
	  unsigned int imap1 = translate_mcu_inputs(buf[5]);
	  unsigned int changed_bitmap = checkSensorInputs(imap1, 0x3f);
	  dprintf("Bootup ready(%x,%d,%d,%x)\n", imap1, hdlock, hdinserted,changed_bitmap);
	  if (changed_bitmap)
	    update_inputmap();
	  unsigned int other_bits = g_input_save & ~g_mcuinputmask;
	  g_input_save = other_bits | (imap1 & g_mcuinputmask);
	  g_bootupsent = 1;
	  break;
	}
      }
...
int appinit()
{
    ...
    if( p_dio_mmap->inputnum<=0 || p_dio_mmap->inputnum>MAXINPUT )
    ...
    if( g_mcuinputnum<=0 || g_mcuinputnum>MAXINPUT )
    ...
    for (i = 0; i < g_mcuinputnum; i++) {
      char sec[12] ;
      unsigned int mask = 0;
      g_sensorConf[i].inverted = 0;
      sprintf( sec, "sensor%d", i+1);
      if (dvrconfig.getvalueint(sec, "inverted")) {
	mask = (1 << i);
	defaultmap |= mask;
	g_sensorConf[i].inverted = 1;
      }
      g_sensorConf[i].lastChanged.tv_sec = 0;
      g_sensorConf[i].lastChanged.tv_usec = 0;
    }
    p_dio_mmap->inputmap = defaultmap & g_mcuinputmask;
    g_input_debounced = g_input_save = defaultmap & g_mcuinputmask;
    fprintf(stderr, "default inputmap:%X,%X\n", defaultmap,g_input_save);
...
void check_temp_logfile_size()
{
  struct stat sb;
  if (!stat(temp_logfile, &sb)) {
    /* delete file if too big */
    if (sb.st_size > (128 * 1024)) {
      unlink(temp_logfile);
    }
  } 
}
...
//                mcu_reboot();
    ...
    while( app_mode ) {
	      ...
	      dprintf("Temperature: hd(%d)\n", i );
            ...
            if( p_dio_mmap->iotemperature > 83 || p_dio_mmap->hdtemperature > 83 ) {
                   ...
                   ((p_dio_mmap->dvrstatus & DVR_RECORD)!=0) )
                {
                    ...
                    stop_dvr_recording();     // stop recording
                    hightemp_norecord = 1 ;
                }
            }
                ...
                if( hightemp_norecord == 1 &&
                   ...
                   ((p_dio_mmap->dvrstatus & DVR_RECORD)==0) )
                {
                    start_dvr_recording();     // start recording
                    hightemp_norecord = 0 ;
		...
		g_startrecordingsent = 1;
		  ...
		  if( p_dio_mmap->dvrpid>0 && 
			...
			kill( p_dio_mmap->glogpid, SIGUSR1 );
		      ...
		      system("[ -f /var/dvr/vter.pid ] && kill -USR1 `cat /var/dvr/vter.pid`");
		      ////////////////////////// continue from here
		...
		if (smartftp_ended) {                
		  ...
		  if (smartftp_ended) {
		    if (g_devicepower_allways_on == 0) {
		      ...
		      if (g_wifi_proto == 2) {
			p_dio_mmap->devicepower=DEVICEOFF_NETON ;
		      }
		      //dprintf("devicepower:%x\n", p_dio_mmap->devicepower);
		    }
		    /*
		    if ((p_dio_mmap->dvrpid == 0) &&
			(p_dio_mmap->dvrcmd == 0)) {
        ...
        if( (runtime - hd_timer)> 500 ) {    // 0.5 seconds to check hard drive
            ...
            if( hdlock && hdkeybounce<KEY_BOUNCE_BASE ) {          // HD lock on
                if( hdpower==0 ) {
                    // turn on HD power
		  if (!mcu_hdpower(1))
		    dprintf("hdpower on\n") ;
		   ...
		   (hdinserted && !pid_tab102 && p_dio_mmap->dvrpid<=0 ))
		  {
		    ...
		    ++hdpower;                    
                    ...
                    {
		      /* reset if HD not detected */
		      if (checkHdDetection()) {
                        dvr_log("Hard drive not detected, system reset!");
                        buzzer( 10, 250, 250 );
		...
		//dvr_log("Debug:hdkeybounce:%d.",hdkeybounce);
                if( hdkeybounce<KEY_BOUNCE_BASE ) {                          // wait for key lock stable
                    hdkeybounce++ ;
                    hd_dvrpid_save=0 ;
                }
                else if( hdkeybounce==KEY_BOUNCE_BASE ){
		    dprintf("hdpower off 1\n") ;
                     hdkeybounce++ ;
			...
			(p_dio_mmap->dvrstatus & DVR_RUN)) {
                ...
                else if( hdkeybounce==(KEY_BOUNCE_BASE+1) ) {
 		    dprintf("hdpower off 2\n") ;
                   if( p_dio_mmap->dvrstatus <= 0 )  {
                        hdkeybounce++ ;
                        //sync();
                        // umount disks
                ...
                else if( hdkeybounce==(KEY_BOUNCE_BASE+2) ) {
                   // do turn off hd power
                    //sync();
		  mcu_hdpower(0); /* this turns power off, too */
                    hdkeybounce++;
                }
                else if( hdkeybounce== (KEY_BOUNCE_BASE+3)) {
                    hdkeybounce++;
                    if( hd_dvrpid_save > 0 ) {
		    ...
		    mcu_reboot(); /* just in case hdpower off didn't work*/
		    exit(1);
        ...
        if( (runtime - panel_timer)> 2000 ) {    // 2 seconds to update pannel
			...
			dprintf("device:%d, power:%d, result:%d\n",
				i, p_dio_mmap->devicepower & (1<<i), xx);
                    }
                }
	...
	unsigned char changed_bitmap = checkSensorInputDebouncing();
	if (changed_bitmap)
	  update_inputmap();
    }
