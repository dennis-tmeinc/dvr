...
void firmwareupload_page()
{
    ...
    if( firmwaremsgfile ) {
        if( firmwareok ) {
            fprintf( firmwaremsgfile, "DVR will reboot itself." );
        }
        else {
...
void mcu_firmwareupload_page()
{
    ...
    if( mcu_upd_pid == 0 ) {
        ...
        // if glog running, kill it
        dvrpidfile=fopen("/var/dvr/glog.pid", "r");
        if( dvrpidfile ) {
            dvrpid=0 ;
            fscanf(dvrpidfile, "%d", &dvrpid);
            fclose( dvrpidfile );
...
int main()
{
    ...
    else if( page && strcmp(page, "/restoredef.html")==0 ) {
        system("./applysetup default 2> /dev/null ");
    }
