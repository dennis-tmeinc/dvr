...
int main()
{
    ...
    if( fvalue ) {
        ...
        // peak(OSD) enable
        ivalue = dvrconfig.getvalueint("io", "showpeak");
        if( ivalue == 1 ) {
            fprintf(fvalue, "\"en_peak\":\"on\",");
        }
    ...
    for( i=1; i<=camera_number; i++ ) {
		...
		fprintf(fvalue, "\"sensor%d\":\"%d\",", ivalue, ivalue );
		if( itrig>0 ) {
    ...
    if( fvalue ) {
        ...
        for( i=1; i<=sensor_number; i++ ) {
            ...
            if( dvrconfig.getvalueint(section, "eventmarker") ) {
                fprintf( fvalue, "\"%s_ev\":\"on\",", section );
            }

            // rising
            if( dvrconfig.getvalueint(section, "on_rising") ) {
                fprintf( fvalue, "\"%s_rising\":\"on\",", section );
            }

            // falling
            if( dvrconfig.getvalueint(section, "on_falling") ) {
                fprintf( fvalue, "\"%s_falling\":\"on\",", section );
            }

            // immobile
            if( dvrconfig.getvalueint(section, "on_immobile") ) {
                fprintf( fvalue, "\"%s_immobile\":\"on\",", section );
            }
        }
        
        // immobile_sensor
        fprintf(fvalue, "\"immobile_sensor\":\"%d\",", 
		dvrconfig.getvalueint( "io", "immobile_sensor" ));

        // immobile_speed
        fprintf(fvalue, "\"immobile_speed\":\"%d\",", 
		dvrconfig.getvalueint( "io", "immobile_speed" ));
	...
	if( value.length()>0 ) {
	  fprintf( fvalue, "\"wifi_mask\":\"%s\",", value.getstring() );
        ...
        // gateway
	value = dvrconfig.getvalue("network", "gateway_1");
	if( value.length()>0 ) {
	  fprintf( fvalue, "\"gateway_1\":\"%s\",", value.getstring() );
	}

	// wifi setup mode
	ivalue = dvrconfig.getvalueint("network", "wifi_proto");
	if( ivalue<0 || ivalue>2 ) {
	  ivalue=0 ;                   // default
	}
	fprintf(fvalue, "\"wifi_proto\":\"%d\",", ivalue );
