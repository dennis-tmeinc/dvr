 ...
 *   1. added DHCP support
 *   2. added "Restore defaults"
 *
 * 01/13/2011 by Harrison
 *   1. trigger values in user friendly format
 *
 * 04/21/2011 by Harrison
 *   1. added EV with stop condition
 *
 */
...
int main(int argc, char **argv)
{
  ...
  if ((argc > 1) && !strcmp(argv[1], "default")) {
    system("cp /davinci/dvr/default.conf /davinci/dvr/dvrsvr.conf");
  } else { 
    // set system_value
    ...
    if( fvalue ) {
        ...
        // peak(OSD) enable
        v = getsetvalue ("en_peak");
        if( v ) {
            dvrconfig.setvalueint( "io", "showpeak", 1 );
        }
        else {
            dvrconfig.setvalueint( "io", "showpeak", 0 );
        }
    ...
    if( fvalue ) {
	    ...
	    int mcuinputnum = 6;
	    if (sensor_number != 14) {
	      mcuinputnum = sensor_number;
	    }
            dvrconfig.setvalueint( "io", "inputnum", sensor_number );
            dvrconfig.setvalueint( "io", "mcuinputnum", mcuinputnum );
        }
        ...
        for( i=1; i<=sensor_number; i++ ) {
            ...
            sprintf( name,   "sensor%d_name", i );
            v = getsetvalue(name);
            if( v ) {
                dvrconfig.setvalue(section, "name", v );
            }
            
            sprintf( name, "sensor%d_inverted", i );
            v = getsetvalue(name);
            if( v ) {
            ...
            sprintf( name, "sensor%d_rising", i );
            v = getsetvalue(name);
            if( v ) {
                dvrconfig.setvalueint(section, "on_rising", 1 );
            }
            else {
                dvrconfig.setvalueint(section, "on_rising", 0 );
            }

            sprintf( name, "sensor%d_falling", i );
            v = getsetvalue(name);
            if( v ) {
                dvrconfig.setvalueint(section, "on_falling", 1 );
            }
            else {
                dvrconfig.setvalueint(section, "on_falling", 0 );
            }

            sprintf( name, "sensor%d_immobile", i );
            v = getsetvalue(name);
            if( v ) {
                dvrconfig.setvalueint(section, "on_immobile", 1 );
            }
            else {
                dvrconfig.setvalueint(section, "on_immobile", 0 );
            }
        }

	v = getsetvalue("immobile_sensor");
	if( v ) {
	  dvrconfig.setvalueint("io", "immobile_sensor", atoi(v) );
	}
	v = getsetvalue("immobile_speed");
	if( v ) {
	  dvrconfig.setvalueint("io", "immobile_speed", atoi(v) );
	}
    }
...
//          # picture quality, 0:lowest, 10:highest
	...
	if( v ) {
	  dvrconfig.setvalue( "network", "eth_ip", v);
	}

        // eth_mask
        v=getsetvalue("eth_mask");
        if( v ) {
	  dvrconfig.setvalue( "network", "eth_mask", v);
        }

        // wifi_ip
        v=getsetvalue("wifi_ip");
        if( v ) {
	  dvrconfig.setvalue( "network", "wifi_ip", v);
        }

        // wifi_mask
        v=getsetvalue("wifi_mask");
        if( v ) {
	  dvrconfig.setvalue( "network", "wifi_mask", v);
        }

        // gateway
        v=getsetvalue("gateway_1");
        if( v ) {
	  dvrconfig.setvalue( "network", "gateway_1", v);
        }
   
        // wifi setup mode
        v=getsetvalue("wifi_proto");
        if( v ) {
	  int value;
	    sscanf(v, "%d", &value);
            if( value<0 || value>2 ) {
            ...
            dvrconfig.setvalueint( "network", "wifi_proto", value );
        }
