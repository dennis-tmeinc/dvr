...
#else
  data->progress.dlspeed = (curl_off_t)
    (my_floatdidf(data->progress.downloaded)/
     (data->progress.timespent>0?data->progress.timespent:1));
#endif
...
#else
  data->progress.ulspeed = (curl_off_t)
    (my_floatdidf(data->progress.uploaded)/
     (data->progress.timespent>0?data->progress.timespent:1));
#endif
...
#else
          data->progress.current_speed = (curl_off_t)
            (my_floatdidf(amount)/((double)span_ms/1000.0));
#endif
        else
...
#else
      result= data->set.fprogress(data->set.progress_client,
                                  my_floatdidf(data->progress.size_dl),
                                  my_floatdidf(data->progress.downloaded),
				  my_floatdidf(data->progress.size_ul),
				  my_floatdidf(data->progress.uploaded));
#endif
      if(result)
