...
int get_peak_data( double *fb, double *lr, double *ud )
{
  *fb = 0;
  *lr = 0;
  *ud = 0;
...
int dio_input( int no )
{
	...
	//fprintf(stderr, "dioinput(%d):%d,%x\n",no,((p_dio_mmap->inputmap)>>no)&1,p_dio_mmap->inputmap);
...
int dio_check()
{
	...
	g_nodiskcheck = p_dio_mmap->nodiskcheck;
        dio_ioprocess_mode = p_dio_mmap->current_mode;
...
void dio_init(int prerun)
{
    ...
    dio_old_inputmap = 0 ;
    dio_ioprocess_mode = APPMODE_RUN ;
    ...
    for( i=0; i<10; i++ ) {
        if( p_dio_mmap->iopid ) {
            ...
            p_dio_mmap->dvrcmd = 0 ;
            p_dio_mmap->dvrstatus = prerun ? 0 : DVR_RUN ;
            return ;		// success 
        }
