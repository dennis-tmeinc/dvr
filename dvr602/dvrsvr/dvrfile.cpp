 ...
 *
 * 1/12/2010 by Harrison
 *   1. Support for remote playback of encrypted video changed
 *
 * 1/29/2010 by Harrison
 *   1. Fixed possible overflow bug in seek()
 *
 */
...
{
    int i;
    int seeks, seekms ;					// seek in milliseconds frome time of file
    if( !isopen() ) {
        return 0;
    }
...
int dvrfile::repairpartiallock()
{
    ...
    for( i=breakindex ; i<m_keyarray.size(); i++ ) {
        // if rec_busy, delay for 2 s
        int busywait ;
        for( busywait=0; busywait<200; busywait++) {
            if( rec_busy==0 ) break ;
            usleep(10000);
        }
