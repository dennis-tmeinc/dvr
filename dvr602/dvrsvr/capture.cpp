...
/*
    ...
    sprintf(section, "camera%d", ch+1);
    dvrconfig.setvalueint( section, "enable", pchannel->Enable);
    dvrconfig.setvalue( section, "name", pchannel->CameraName);
    dvrconfig.setvalueint( section, "resolution", pchannel->Resolution);
    dvrconfig.setvalueint( section, "recordmode", pchannel->RecMode);
    dvrconfig.setvalueint( section, "quality", pchannel->PictureQuality);
    dvrconfig.setvalueint( section, "framerate", pchannel->FrameRate);
    
    // triggering sensor and osd
    for(i=0; i<4; i++) {
        sprintf(trigger, "trigger%d", i+1);
        if( pchannel->SensorEn[i]==0 ) {
            dvrconfig.removekey(section, trigger);
        }
        else {
            dvrconfig.setvalueint( section, trigger, pchannel->Sensor[i]+1);
        }
        sprintf(sensorosd, "sensorosd%d", i+1);
        if( pchannel->SensorOSD[i]==0 ) {
            dvrconfig.removekey(section, sensorosd);
        }
        else {
            dvrconfig.setvalueint( section, sensorosd, pchannel->Sensor[i]+1);
        }
    }
    
    dvrconfig.setvalueint( section, "prerecordtime", pchannel->PreRecordTime);
    dvrconfig.setvalueint( section, "postrecordtime", pchannel->PostRecordTime);
    
    // recording alarm
    dvrconfig.setvalueint( section, "recordalarm", pchannel->RecordAlarm);
    dvrconfig.setvalueint( section, "recordalarmen", pchannel->RecordAlarmEn);
    dvrconfig.setvalueint( section, "recordalarmpattern", pchannel->RecordAlarmPat);
    
    // video lost alarm
    dvrconfig.setvalueint( section, "videolostalarm", pchannel->VideoLostAlarm);
    dvrconfig.setvalueint( section, "videolostalarmen", pchannel->VideoLostAlarmEn);
    dvrconfig.setvalueint( section, "videolostalarmpattern", pchannel->VideoLostAlarmPat);
    
    // motion detection
    dvrconfig.setvalueint( section, "motionalarm", pchannel->MotionAlarm);
    dvrconfig.setvalueint( section, "motionalarmen", pchannel->MotionAlarmEn);
    dvrconfig.setvalueint( section, "motionalarmpattern", pchannel->MotionAlarmPat);
    dvrconfig.setvalueint( section, "motionsensitivity", pchannel->MotionSensitivity);
    
    // GPS
    dvrconfig.setvalueint( section, "gpsen", pchannel->GPS_enable );
    dvrconfig.setvalueint( section, "showgps", pchannel->ShowGPS );
    dvrconfig.setvalueint( section, "gpsunit", pchannel->GPSUnit );
    
    // bitrate
    dvrconfig.setvalueint( section, "bitrateen", pchannel->BitrateEn);
    dvrconfig.setvalueint( section, "bitrate", pchannel->Bitrate);
    dvrconfig.setvalueint( section, "bitratemode", pchannel->BitMode);
    
    // ptz address
    dvrconfig.setvalueint( section, "ptzaddr", pchannel->ptz_addr);
    
    // picture control
    dvrconfig.setvalueint( section, "brightness", pchannel->brightness);
    dvrconfig.setvalueint( section, "contrast", pchannel->contrast);
    dvrconfig.setvalueint( section, "saturation", pchannel->saturation);
    dvrconfig.setvalueint( section, "hue", pchannel->hue);

    dvrconfig.setvalueint( section, "keyinter", pchannel->key_interval);

    // no audio
    dvrconfig.setvalueint( section, "disableaudio", pchannel->disableaudio);
...
void capture::loadconfig()
{
    ...
    m_attr.showpeak=dvrconfig.getvalueint( "io", "showpeak" );
...
void capture::update(int updosd)
{
	...
	if( dio_ioprocess_mode != APPMODE_STANDBY ) {				// don't record video lost event in standby mode. (Requested by Harrison)
...
static int  osdlines3_x = 160 ;
static int  osdlines3_y = 448 ; // was 440
...
void capture::updateOSD()
{
    ...
    if( m_attr.showpeak ) {
      // prepare line 3 (peak value)
      i=0;
      osd.osdline[line][i++]= 0 ;
      osd.osdline[line][i++]= osdlines3_y ;          
      ...
      sprintf( osdbuf, "%c%5.2lf,%c%5.2lf,%c%5.2lf",
	       ...
	       g_lr >= 0 ? g_lr : -g_lr,
	       g_ud >= 1.0 ? 'D' : 'U',
	       g_ud >= 900 ? 0.0 : (g_ud >= 1.0 ? (g_ud - 1.0) : 1 - g_ud));
      k=osdbuf ;
      while( *k ) {
    ...
    // prepare line for gpslocation
    i=0;
    osd.osdline[line][i++]= osdlines3_x ;
    osd.osdline[line][i++]= osdlines3_y ;          
