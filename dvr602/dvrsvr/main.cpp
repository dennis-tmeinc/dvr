...
static char deftmplogfile[] = "/davinci/dvrlog";
...
void dvr_cleanlog(FILE * flog)
{
    ...
    if( pos>logfilesize ) {
        ...
        line.setbufsize(1024) ;
        fgets( line.getstring(), 1024, flog );	// skip this line.
        while (fgets(line.getstring(), 1024, flog)) {
            if( line.length()>2 ) {
                alog.add( line );
            }
        }
...
int dvr_getsystemsetup(struct system_stru * psys)
{
    ...
    // event marker not supported anymore
    psys->eventmarker=0 ;
    psys->eventmarker_enable=0 ;
...
void sig_check()
{
    ...
    //dvr_log("Debug(sig_check:%x).", sigmap);
    if( sigmap & (1<<SIGTERM) ) 
    {
...
void app_init(int partial)
{
    ...
    if( logfilesize< (100*1024 ) || logfilesize>(10*1024*1024) ) {
