...
static int repairfile(const char *filename)
{
    ...
    if (vfile.open(filename, "r+") == 0) {	// can't open it
      dvr_log("Repair file open error: %s", filename);
...
void copy_gforce_file(int dst)
{
  ...
  strcat(dstdir, "/smartlog");
  
  snprintf(cmd, sizeof(cmd),
	   "mkdir -p %s && /davinci/dvr/gforce.sh %s",
	   dstdir, dstdir);
  fprintf(stderr, "%s\n", cmd);
  system(cmd);
}

void copy_smartftp_debug(int src, int dst)
{
  char filename[512];
  ...
  copy_file_to_path(filename, dstdir, 1, 0);
}
 ...
 *        (changed on 20100722:copy from bigger to smaller)
 * L to N: never copy (changed on 20100722:copy from bigger to smaller)
 * N,L to none: copy
 */
	...
	if (isHybridDisk(&backupDisk, &recordingDisk)) {
	  if ((backupDisk >= 0) && (recordingDisk >= 0)) { /* always */
	    ...
	    if ((errno == 0) && (setpriority(PRIO_PROCESS,0,10) == 0))
	      prio_set = 1;
	    // Don't do this here, it'll be copied only at IGN OFF
#if 0
	    copyPendingFiles(disk_disklist[recordingDisk].basedir.getstring(),
			     disk_disklist[backupDisk].basedir.getstring());
#endif
	    if (prerun) {
	      copy_gforce_file(recordingDisk);
	      sync_logfiles_for_hybrid(recordingDisk, backupDisk, 0);
	      copy_smartftp_debug(backupDisk, recordingDisk);
	    }
	    if (prio_set) {
...
int do_disk_check()
{
  ...
  if (recdisk >= 0) {
    fprintf(stderr, "Found recording disk:id(%d)\n", recdisk);
#if 0 /* done in disk_findrecdisk(1) */
