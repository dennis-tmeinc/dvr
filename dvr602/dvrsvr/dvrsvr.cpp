...
#endif    
    fprintf(stderr, "Receive cmd : %d - payload : %d\n", m_req.reqcode, m_recvlen );
...
void dvrsvr::Req2GetJPEG()
{
    ...
    if( ch >=0 && ch < cap_channels ) {
        // problem is JPEG capture doesn't work
      // if called from here, JPEG capturing only works in main thread
...
void dvrsvr::Req2GetGPS()
{
  ...
  memset(&g2, 0, sizeof(struct gps_t));
  get_gps_data(&g);

  if (g.gps_valid) {
    ans.anscode = ANS2GETGPS;
    ans.anssize = sizeof(struct gps_t);
  }

  Send(&ans, sizeof(ans));

  if (g.gps_valid) {
    g2.gps_speed = g.gps_speed;
    g2.gps_direction = g.gps_direction;
    g2.gps_latitude = g.gps_latitude;
    g2.gps_longitud = g.gps_longitud;
    g2.gps_gpstime = g.gps_gpstime;

    Send(&g2, sizeof(struct gps_t));
  }
}
