 ...
 * 
 * 01/11/2010 by Harrison
 *   1. Fix: gps data handling error.
 * 
 * 07/07/2010 by Harrison
 *   1. Fix: buzzer error on peak value update.
 *   2. No GPS output setup(sometimes no gps output at all).
 * 
 * 07/14/2010 by Harrison
 *   1. Fix: no sensor log update after SIG2.
 *
 * 01/13/2011 by Harrison
 *   1. log GPS speed > 2.0km only (was 1.8km).
 *   2. show delta value for GForce
 *
 * 04/19/2011 by Harrison
 *   1. Fix: extra sensor logging with log type 15.
 */
...
#define SENSORLOG_INTERVAL 20000
...
void set_peak_data(double fb, double lr, double ud)
{
  struct sembuf sb = {0, -1, 0};
  ...
  p_dio_mmap->gforce_forward_d = (float)fb;
  p_dio_mmap->gforce_right_d = (float)lr;
  p_dio_mmap->gforce_down_d = (float)ud;

  sb.sem_op = 1;
  semop(semid, &sb, 1);
}

void get_gforce_data(float *fb, float *lr, float *ud, int *counter)
{
  struct sembuf sb = {0, -1, 0};
  ...
  *fb = p_dio_mmap->gforce_forward_0;
  *lr = p_dio_mmap->gforce_right_0;
  *ud = p_dio_mmap->gforce_down_0;
  *counter = p_dio_mmap->gforce_log0;
...
int gps_readdata2(struct gps_data * gpsdata)
{
  ...
  if (bytes > 0) {
	...
	if( gpsfifo>0 ) {
	  int len = strlen(rmc_line);
	  if( write( gpsfifo, rmc_line, len )!=len ) {
	    close(gpsfifo);
	    gpsfifo=0;
	  }
	}
    ...
    usleep(100000); /* hardware problem */
  }
...
int gps_logprintf(int logid, char * fmt, ...)
{
  ...
  if( (validgpsdata.speed * 1.852) < 2.0 ) { /* log only 2km and above */ 
    gps_logclose ();
  }
...
int	sensor_log()
{
  ...
  // log g sensor value
  get_gforce_data(&fb, &lr, &ud, &glog0);
  if (glog0 != glog0_save) {
    glog0_save = glog0;
    if( validgpsdata.year >= 1900 )
      gps_logprintf(16, ",%.2f,%.2f,%.2f", fb, lr, ud);
    got_peak = 1;
  }
 
  if (got_peak) {
    //clear_gps_data();
    set_peak_data(fb, lr, ud);
    peak_err = 0;
  } else {
    peak_err++;
    if (peak_err > (3000000 / SENSORLOG_INTERVAL)) {
      peak_err = 0;
      set_peak_data(0, 0, 999.9);
    }
  }
  ...
  if( validgpsdata.year >= 1900 ) {
	...
	if (i < 6) {
	  gps_logprintf( (sv)? (sensorid+i*2):(sensorid+i*2+1), "" );
	} else {
	  extra = 1;
	}	 
	sensor_value[i]=sv ;
    ...
    if (extra && p_dio_mmap->inputnum > 6) {
      unsigned short extra_map = 0; /* S23...S7 */
      for (i = 0; i < p_dio_mmap->inputnum - 6; i++) {
	if (sensor_value[6 + i])
	  extra_map |= (1 << i);
      }
      gps_logprintf(15, ",%04X", extra_map);
    }
  }
...
static int writePeakData()
{
...
#if 0 // let sensor_thread handle writing.
  if( p_dio_mmap->gforce_log0==0 ) {
    ...
    p_dio_mmap->gforce_right_1 = lr;
    p_dio_mmap->gforce_forward_1 = fb;
    p_dio_mmap->gforce_down_1 = ud;
...
int appinit()
{
  ...
  if (gps_port_handle == -1) {
    exit(1);
  }

  if (strstr(gps_port_dev, "USB")) {
    /* this helps */
