/* --- Changes ---
 *
 * 06/08/2010 by Harrison
 *   1. changed the way dvrsvr is started.
 *   2. support for Hybrid disk.
 */
...
void read_config()
{
  ...
  p = cfg_get_str("system", "hostname");
  if (p) {
    strncpy(g_hostname, p, sizeof(g_hostname));
   }

  p = cfg_get_str("system", "iomapfile");
  if (p) {
    strncpy(dvriomap, p, sizeof(dvriomap));
  }
		     ...
		     char *dest_dir)
{
  ...
  if (!fpnew_k) {
    writeDebug("fopen3:%s(%s)", strerror(errno), newfile);
    retval = -1;
    goto error_exit;
  }
...
int wait_until_glog_stopped() {
  FILE *fp;
  int retry = 0;

  while(retry < 10) {
    sleep(1);
    fp = fopen("/var/dvr/glog.pid", "r");
    if (!fp) {
      return 0;
    }
...
pid_t stop_dvrsvr() {
  ...
  strncpy(buf, "[ -f /var/dvr/udhcpc.pid ] && kill `cat /var/dvr/udhcpc.pid`", sizeof(buf));
  system(buf);  
...
int start_dvrsvr(pid_t pid) {
  ...
  if (p_dio_mmap->dvrpid > 0) {
    // wait until dvrsvr is ready
    for( i=0;i<100; i++ ) {
      if( (p_dio_mmap->dvrstatus & DVR_RUN) && (p_dio_mmap->dvrcmd == 0) ) 
	break;
      usleep(100000);
    }
  ...
  system("/davinci/dvr/setnetwork");  

  /* start glog again */
  system("/bin/insmod /davinci/usbserial.ko");
  system("/bin/insmod /davinci/mos7840.ko");
  system("/davinci/dvr/glog");
...
int main(int argc, char **argv)
{
  ...
  writeDebug("busname:%s, usbroot:%s", g_hostname, usbroot);
  ...
  if (time_range) {
    add_time_range_to_list(time_range, g_hostname);
    free(time_range);
  }

  scan_recording_disks(mountdir, g_hostname);
  upload_files(usbroot, g_hostname);
