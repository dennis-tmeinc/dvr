...
int md5check( char * checkfilename, int print )
{
    ...
    char fbuf[512], filepath[256], fullname[512] ;
    FILE * checkfile ;
    int i, j, l ;
    get_filepath(checkfilename, filepath, sizeof(filepath));
    ...
    if( checkfile ) {
	    ...
	    sprintf(fullname, "%s/%s", filepath, fbuf);
            FILE * mfile = fopen( fullname, "rb" );
            if( mfile ) {
