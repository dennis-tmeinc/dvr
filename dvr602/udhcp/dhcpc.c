...
#define MAX_TRY 5 /* added by hyun */
...
#endif
{
	...
	for (;;) {
		...
		if (retval == 0) {
			...
			case INIT_SELECTING:
			  //if (packet_num < 3) {
				if (packet_num < MAX_TRY) {
					...
					timeout = now + ((packet_num == (MAX_TRY-1)) ? 4 : 2);
					packet_num++;
				} else {
			...
			case REQUESTING:
			  //if (packet_num < 3) {
				if (packet_num < MAX_TRY) {
					...
					timeout = now + ((packet_num == (MAX_TRY-1)) ? 10 : 2);
					packet_num++;
				} else {
			...
			/* added by hyun, to avoid ip confict */
			if (memcmp(packet.chaddr, client_config.arp, 6)) {
			  DEBUG(LOG_INFO,
				"Ignoring MAC %02x.%02x.%02x.%02x.%02x.%02x",
