...
void lock_semaphore(int semid)
{
  ...
  sb.sem_num = 0; /* 1st semaphore in the set */
  sb.sem_op = -1; /* lock */
  sb.sem_flg = 0; /* no flag */
  semop(semid, &sb, 1);
}
...
int send_ipc_message_to_modem(int sender,int command,
		     int argc, unsigned char *argv)
{
...
#if 0  
  struct gps_t* ptemp;
  ptemp=(struct gps_t*)argv;
  printf("==valid:%d time:%d latitude:%f longtitude:%f speed:%f direction:%f\n",ptemp->valid,ptemp->fix_time, \
        ptemp->latitude,ptemp->longitude,ptemp->speed,ptemp->direction
  );
#endif  
  ...
  strcpy(name.sun_path, SOCKPATH_MODEM);

  char txbuf[128];
  int bi = 0;
  txbuf[bi++] = sender; /* sender id */
  txbuf[bi++] = command; /* command number */
  txbuf[bi++] = (argc << 8) & 0xff; /* data size HI */
  txbuf[bi++] = (argc << 0) & 0xff; /* data size LO */
//  printf("\n");
  for (i = 0; i < argc; i++) {
    txbuf[bi++] = argv[i]; /* command arg */
//    printf("%0x ",argv[i]);
  }  
//  printf("\n");
  
  lock_semaphore(semid);
  if (sendto(sock, txbuf, bi, 0,
	     (struct sockaddr *)&name, sizeof(struct sockaddr_un)) < 0) {
    if (errno != ENOENT) {
      fprintf(stderr, "ipc send error:%d:%s\n", errno, strerror(errno));
    }
    ret = 1;
  }
...
int dio_get_doortriggered()
{
    if( p_dio_mmap && p_dio_mmap->iopid ) {
    //   printf("doortriggered:%d\n",p_dio_mmap->doortriggered);
       if(p_dio_mmap->doortriggered==1)
         return 1;
    }
    return 0;
}

void dio_clear_doortriggered()
{
    if( p_dio_mmap && p_dio_mmap->iopid ){
        p_dio_mmap->doortriggered=0;
    }
}
