...
//    stop();
	...
	SetSecondStreamEnable(m_hikhandle-1,SecondStreamEn);	
	if(SecondStreamEn){
            SetSecondStreamBitrate(m_hikhandle-1,Bitrate_Second);
	    SetSecondStreamFramerate(m_hikhandle-1,FrameRate_Second);
	    SetSecondStreamResolution(m_hikhandle-1,Resolution_Second);	
	    SetSecondStreamInterval(m_hikhandle-1,FrameRate_Second*3);
	}
#if 0	
