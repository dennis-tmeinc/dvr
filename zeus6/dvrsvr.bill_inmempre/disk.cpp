    ...
    while( dfind.find() ) {
        ...
        if( dfind.isdir() ) {
            ...
            if (strcmp(&pathname[l - 4], ".266") == 0) {
                res=1 ;
                break;
            } else if(strcmp(&pathname[l - 4], ".evn") == 0){
	       res=1;
	       break;
	    } else if(strcmp(&pathname[l - 4], ".emk") == 0){
	       res=1;
	       break;
	    } else if(strcmp(&pathname[l - 4], ".crh") == 0){
	       res=1;
	       break;
	    } 	
        }
...
int copyeventfile(char *from,char* to)
{
   int ret=0;
   char basefrom[256];
   char baseto[256];
   char dirnameTo[256];
   char dirnameFrom[256];
   char filenameTo[256];
   char cmd[1024];
   char * pathname;
  // dvr_log("inside copy event files: from:%s to:%s",from,to);
   sprintf(basefrom,"%s/event",from);
   sprintf(baseto,"%s/event",to);
   mkdir(baseto, S_IFDIR | S_IRWXU | S_IRGRP | S_IROTH);
   dir_find dfind(basefrom);
   while(dfind.find()){
       dio_get_nodiskcheck();
       if (!g_nodiskcheck)
         break;
       pathname = dfind.filename();
       if(dfind.isdir()){
           sprintf(dirnameTo,"%s/%s",baseto,pathname);
	   mkdir(dirnameTo, S_IFDIR | S_IRWXU | S_IRGRP | S_IROTH);
	   pathname=dfind.pathname();
	   dir_find dfind1(pathname);
	   while(dfind1.find()){
	       if(dfind1.isfile()){		   
		   pathname=dfind1.filename();		   
		   sprintf(filenameTo,"%s/%s",dirnameTo,pathname);		   
		   pathname=dfind1.pathname();		   
		   sprintf(cmd,"cp -f %s %s",pathname,filenameTo);
		   system(cmd);
		   remove(pathname);
		  // dvr_log("%s is copied to %s",pathname,filenameTo);
	       }
	       dio_get_nodiskcheck();
	       if (!g_nodiskcheck)
		 break;	     
...
void merge_eventlog()
{
    ...
    for(i=0;i<disk_disklist.size();++i){
          if(i!=mCur){
	     copyeventfile(disk_disklist[i].basedir.getstring(),disk_disklist[mCur].basedir.getstring());
	  }
    }
    return;
}
...
void disk_check()
{
    ...
    if(disk_disklist.size()!=numberoftotalpartitions){
        if(!errLedFlashed) {
	       if(disk_disklist.size()<numberoftotalpartitions){
	          dvr_log("missed SD flash");
	       } else {
		  dvr_log("Too many SD flashes");
	       }
	       errLedFlashed=1;
	       dio_flasherrled(1);	  
	}
