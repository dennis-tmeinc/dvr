...
void tab102_reset()
{
   ...
   if(pid==0){
      ...
      execl( "/dav/dvr/tab102", "/dav/dvr/tab102","-reset",
	   NULL 
	   );
...
void tab102_setup()
{
   int pid=fork();
   if(pid==0){
      execl( "/dav/dvr/tab102", "/dav/dvr/tab102","-st",
	   NULL 
	   );
...
{  
    ...
    format_id=fork();
    if(format_id==0){     
       execlp("/dav/mkdosfs", "mkdosfs", "-F","32", devname, NULL );  
       exit(0);
    } else if(format_id>0){    
        int status=-1;
	 ...
	 //   printf("hdkeybounce:%d\n",hdkeybounce);
		...
		//dvr_log("hdinserted:%d",hdinserted);
			...
			if( ++hdpower>600)  
			{
			  ...
			  if(mTempState==3)
			      system("/dav/dvr/tdevmount /dav/dvr/tdevhotplug");
			}
		    } 		    
		    else {
		      hdpower=1;
		 ...
		 //  dvr_log("key lock off");	    
			...
			dvr_log("umount disks");
                ...
                else if( hdkeybounce==8 ) {               //12
                    // do turn off hd power
                    sync();	
                    mcu_hdpoweroff();       // FOR NEW MCU, THIS WOULD TURN OFF SYSTEM POWER?!
                }
                else if( hdkeybounce<13&&hdkeybounce>8 ) {
		     mcu_reboot ();
                } else if(hdkeybounce>=13) {
		      p_dio_mmap->outputmap &= ~ HDLED ;		      
		      if( hdlock ) {
                        hdkeybounce=0 ;
