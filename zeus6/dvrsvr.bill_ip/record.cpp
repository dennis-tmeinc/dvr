...
static int rec_fifo_size =8000000; // 4000000 ;
...
rec_channel::rec_channel(int channel)
{
    ...
    cameraType=   dvrconfig.getvalueint(cameraname, "type");	
    m_triggersensor = 0 ;
    ...
    if(cameraType){
 	m_width=GetIPVideoWidth(m_channel);
	m_height=GetIPVideoHight(m_channel);      
    } else {
...
#ifdef 	FRAME_DEBUG 
           dvr_log("ch%d one frame droped0\n",m_channel);
#endif	   
           return;
       }     
    }
    if( m_fifosize>rec_fifo_size ){

       if(pframe->frametype == FRAMETYPE_VIDEO){
	  // framedroped=1;
#ifdef 	FRAME_DEBUG 	   
           dvr_log("ch%d one frame droped1\n",m_channel);
#endif	   
           return;
       }       
    }
    ...
    if(nfifo==NULL){
       //  printf("No buffer in addfifo\n");
         ...
         if(nfifo==NULL){
	      if(pframe->frametype == FRAMETYPE_KEYVIDEO){
#ifdef 	FRAME_DEBUG 
                 dvr_log("ch%d one key frame droped due to no buffer for fifo",m_channel);
#endif
		framedroped=1;
	      }
              return;
...
void rec_channel::recorddata(rec_fifo * data)
{
	...
	if(cameraType){
	    m_width=GetIPVideoWidth(m_channel);
	    m_height=GetIPVideoHight(m_channel); 
	    m_framerate=GetIPVideoFrameRate(m_channel);
	} else {
	    m_width=GetVideoWidth(m_channel);
	    m_height=GetVideoHight(m_channel);  
	}	 
