...
protected:	
	...
	int  m_reqclose ;					// 1: if socket close requested from tunnel
...
public:	
	...
	size_t wdatafunc( void * ptr, size_t size )
	{
		char * p = (char *)ptr ;
		while( size>0 && m_llen < sizeof(m_line) ) {
			m_line[m_llen] = *p ;
			if( *p == '\n' ) { 				// end of line
				// to process the command
				m_line[m_llen] = 0 ;
				switch( m_line[0] ) {
					case 'c' :			// this is the only command supported now
						// <cmd:c> <tunnel id> [host] [port]\r\n
						// create new connection
						char id[256] ;
						char host[256] ;
						int port ;
						if( sscanf( m_line+1, "%s %s %d", id, host, &port)==3 ) {
	...
	static size_t s_wdatafunc( void *ptr, size_t size, size_t nmemb, void *userdata)
	{
		return ((connect_curl *)userdata)->wdatafunc(ptr, size*nmemb);
	}
	...
	void request() {
		...
		if( getphonenumber( phonenumber.expand(128) ) ) {
			...
			curl_easy_setopt(m_curl, CURLOPT_WRITEFUNCTION,  connect_curl::s_wdatafunc);
			m_llen = 0 ;
