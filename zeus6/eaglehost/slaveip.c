...
int main(int argc, char * argv[] )
{
...
#if INFO_SHOW
               printf("Recv host ip.\n");
               printf("before ipmsg.msgcode=%d\n",ipmsg.msgcode);
#endif
               if(ipmsg.msgcode == 10002 || 
                 memcmp( ipmsg.hostid, recvmsg.hostid, sizeof( recvmsg.hostid))==0 ) {
                   ipmsg=recvmsg ;
                   res = 0 ;
                   break ;
               }
               else if( memcmp( &sendmsg, &recvmsg, sizeof(recvmsg) )==0 ) {	
                   // see if we repeatly recveive same setip msg (from same host)
                   rept++ ;
                   if( rept>=10 ) {
                       ipmsg=recvmsg ;
                       break;
    ...
    for(;;) {
        if( net_recvok( fd, 20000000 ) ) {		// 20 seconds:w
            ...
            if( res==sizeof(recvmsg) &&
               recvmsg.msgsize == sizeof( struct eagleip_message ) ) {
                   if( memcmp( recvmsg.hostid, ipmsg.hostid, sizeof(ipmsg.hostid) )==0 ) {		
                        // msg from my host
                       if( recvmsg.msgcode == 1 ) {			
                           // set ip
                           // set ip address 
                           ipmsg = recvmsg ;
                           ip = setip( &ipmsg );
                       ...
                       if( recvmsg.ipaddr == ipmsg.ipaddr ) { 
                          // ip address conflict!!!!
