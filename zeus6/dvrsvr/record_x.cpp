...
static int rec_minimum_inmem = 30000 ;	// 30 M
...
class rec_channel {
	...
	public:
		...
		int fifoswapout(struct rec_fifo *fifo) {
			if( fifo && fifo->swappos<=0 && fifo->buf && fifo->bufsize>0 ) {
		...
		int isfifoswap(struct rec_fifo *fifo) {
			return ( fifo != NULL && fifo->buf == NULL && fifo->bufsize>0 && fifo->swappos>0 ) ;
		}
...
void rec_channel::onframe(cap_frame * pframe)
{
       ...
       dio_standby_mode )
    {
		...
		if( g_memfree < 30000 && m_recstate == REC_PRERECORD ) {
			// to mark swapout of this frame
			nfifo->swappos = -1 ;
		}
		putfifo( nfifo );
	}
...
int rec_channel::dorecord()
{
	...
	if( mem_available() < 21000 ) {	// critical low memory !!!
		// drop all fifos
		dvr_log("Very low free memory, %d bytes of channel % fifos dropped!", clearfifo(), m_channel);
		return 0 ;
	}
    
    // get out of in-memory pre-recording
    if( m_recstate >= REC_RECORD && m_fifohead !=NULL && m_fifohead->rectype < m_recstate ) {
	...
	if( m_recstate == REC_PRERECORD && m_inmemprerecord )
	{
		dvrtime now ;
		time_now(&now);
		...
		if( g_memfree < rec_minimum_inmem ) {
    ...
    if( fifo->frametype == FRAMETYPE_KEYVIDEO ) {
