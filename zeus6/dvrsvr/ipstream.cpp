...
static void IPStreamReadCallBack(CALLBACK_DATA CallBackData,void* context)
{
	...
	Ipstream_capture * ipchannel = NULL ;
	if( channel>=0 && channel<MAXCHANNEL ) {
		ipchannel = Ip_capture_array[channel] ;
	}
    if(ipchannel){
       ipchannel->streamcallback(CallBackData);
    }
}
...
//    SetIPVideoKeyframeInterval(m_ipchannel,m_attr.FrameRate);
    SetIPVideoKeyframeInterval(m_ipchannel,m_attr.key_interval);
