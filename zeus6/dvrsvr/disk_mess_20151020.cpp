...
void disk_scanalldisk_x(int partial)
{
...
void disk_pw_mount( char * dev, char * disk, int disknum )
{
	string diskpath ;
	string diskbasepath ;
	string mountcmd ;
	sprintf( diskpath.setbufsize(300), "/var/dvr/%s", disk );
	mkdir( diskpath, 0777 );

	sprintf( diskpath.setbufsize(300), "/var/dvr/%s/%s", disk, dev );
	mkdir( diskpath, 0777);
	
	sprintf(mountcmd.setbufsize(500), "mount /dev/%s %s -o shortname=winnt", dev, (char *)diskpath );
	system( mountcmd );
	...
	if( disknum == 1 ) {
		disk_pw_disk1 = (char *)diskpath ;
		disk_pw_disk1_base = (char *)diskbasepath ;
	}
	else if( disknum == 2 ) {
		disk_pw_disk2 = (char *)diskpath ;
		disk_pw_disk2_base = (char *)diskbasepath ;
	}
	else if( disknum == 3 ) {
		disk_pw_disk3 = (char *)diskpath ;
		disk_pw_disk3_base = (char *)diskbasepath ;
	}
}
		...
		if( disk_pw_disk2_mounted == 0 ) {
			...
			if( t-disk_pw_inittime < 300000 ) {	// < 5 minutes
				if( disk_pw_diskdev( 2, dev )>0 ) {
					...
					disk_pw_mount( (char *)dev, "disks", 2 );
				...
				if( disk_pw_disk1_mounted == 0 ) {
					if( disk_pw_diskdev( 1, dev )>0 ) {
						// mount disk1 to /var/dvr/disks/${dev}
						disk_pw_mount( (char *)dev, "disks", 1 );
						disk_pw_disk1_mounted = 1 ;
						dvr_log("JIC mode, disk2 not available, disk1 mounted.");
					}
				}
			}
		}
			...
			if( disk_pw_disk1_mounted == 0 ) {
				if( disk_pw_diskdev( 1, dev )>0 ) {
					...
					//   mount to 'disks', so it can be play back
					disk_pw_mount( (char *)dev, "disks", 1 );
					disk_pw_disk1_mounted = 1 ;
					dvr_log("JIC mode, disk1 mounted for archiving.");

					// start archive process
					// ...    run bgarch disk2 disk1 2 1    ( lock only, unlock _L_ )
					disk_arch_process1 = disk_pw_runbackup( disk_pw_disk2, disk_pw_disk1, 2, 1 );
			...
			if( disk_pw_disk2_mounted == 0 ) {
				...
				if( t-disk_pw_inittime < 300000 ) {	// < 5 minutes
					if( disk_pw_diskdev( 2, dev )>0 ) {
						...
						// force continue recording
						rec_trigger = 1;
						dvr_log("Auto Backup mode, run as JIC mode, disk2 mounted, record all video in L mode.");
					}
				}
				else {
					// can't mount disk2 for 5 minute, mount disk1 as mode 1 ( one disk mode )
					if( disk_pw_disk1_mounted == 0 ) {
						if( disk_pw_diskdev( 1, dev )>0 ) {
							...
							disk_pw_mount( (char *)dev, "disks", 1 );
							disk_pw_disk1_mounted = 1 ;
							dvr_log("Auto Backup mode, run as JIC mode, disk2 not available, disk1 mounted.");
						}
					}
				}
			}
				...
				if( disk_pw_disk1_mounted == 0 ) {
					if( disk_pw_diskdev( 1, dev )>0 ) {
						// mount disk1 to /var/dvr/arch/${dev}
						disk_pw_mount( (char *)dev, "arch", 1 );
						disk_pw_disk1_mounted = 1 ;
						// start archive process
						// ...    run bgarch disk2 disk1 2 1    ( lock only, unlock _L_ )
						disk_arch_process1 = disk_pw_runbackup( disk_pw_disk2, disk_pw_disk1, 2, 1 );

						dvr_log("Auto Backup mode, run as JIC mode, disk1 mounted for archiving.");
			...
			if( disk_pw_disk2_mounted ) {
				...
				if( disk_pw_disk3_mounted == 0 ) {
					if( disk_pw_diskdev( 3, dev )>0 ) {
						...
						disk_pw_mount( (char *)dev, "autobak", 3 );
						...
						disk_arch_process2 = disk_pw_runbackup( disk_pw_disk2, disk_pw_disk3, 1, 2 );
		...
		if( t-disk_pw_inittime < 300000 ) {	// < 5 minutes
			...
			if( disk_pw_disk1_mounted == 0 && disk_pw_disk2_mounted == 0 ) {
				if( disk_pw_diskdev( 2, dev )>0 ) {
					...
					disk_pw_mount( (char *)dev, "disks", 2 );
					disk_pw_disk2_mounted = 1 ;
					dvr_log("One disk mode, disk1 not available, disk2 mounted.");
				}				
			}
		}
	}
...
int  disk_pw_deleteoldfile( char * disk, int lockfile ) 
{
	...
	if( disk_olddestfile( disk, &oldesttime, &oldestfilename, lockfile ) ) {
		if( oldestfilename.length()>1 ) {
			int res = (remove( (char *) oldestfilename )==0) ;
			if( res ) {
...
// return 1 success
int  disk_pw_checkdiskspace( char * disk ) 
{
	int freespace = disk_freespace(disk);
	int freeratio = disk_freeratio(disk);
	if( freespace < 500 || freeratio < 6 ) {
		// don't remove _L_ file
		return disk_pw_deleteoldfile(disk, 0);
	}
	return 1 ;
}
...
{
	int res ;
	if( disk_pw_disk1_mounted ) {
		res = disk_pw_checkdiskspace( (char *)disk_pw_disk1 ) ;
		if( res==0 ) {
			dvr_log("Disk 1 space might be full.");
			disk_pw_disk1_full=1 ;
		}
	}
	if( disk_pw_disk2_mounted ) {
		res = disk_pw_checkdiskspace( (char *)disk_pw_disk2 );
		if( res==0 ) {
			dvr_log("Disk 2 space might be full.");
			disk_pw_disk2_full=1 ;
		}
	}
	if( disk_pw_disk3_mounted ) {
		res = disk_pw_checkdiskspace( (char *)disk_pw_disk3 );
		if( res==0 ) {
			dvr_log("Disk 3 space might be full.");
			disk_pw_disk3_full=1 ;
		}
	}
}
...
{
	if( disk_pw_recordmethod >= 1 ) {		// Just In Case recording & auto backup
		if( locked ) {
			if( disk_pw_disk1_mounted ) {
				return (char *)disk_pw_disk1_base ;
			}
			else if( disk_pw_disk2_mounted ) {
				return (char *)disk_pw_disk2_base ;
			}
			else 
				return NULL ;
		}
